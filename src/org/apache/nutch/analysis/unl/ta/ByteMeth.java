...
public class ByteMeth
{
	...
	// remove last c chars from b. append r to b
	...
	{
	////System.out.println("addArrayRules");
		byte[] a = null;
		...
		} else if (((b[b.length - 1] == 1) || (b[b.length - 1] == 2)
		  || (b[b.length - 1] == 6) || (b[b.length - 1] == 7)
		  || (b[b.length - 1] == 10) || (b[b.length - 1] == 12))
		  && ((c[0] > 0 && c[0] < 13)))
		{
			a = new byte[b.length + c.length + 1];
		...
		} else if (((b[b.length - 1] == 3) || (b[b.length - 1] == 4)
		  || (b[b.length - 1] == 9) || (b[b.length - 1] == 11)
		  || (b[b.length - 1] == 24))
		  && (c[0] > 0 && c[0] < 13))
		{
			a = new byte[b.length + c.length + 1];
			...
			a[b.length] = 24;
			System.arraycopy(c, 0, a, b.length + 1, c.length);
		} else if (((b[b.length - 1] == 19) || (b[b.length - 1] == 26))
		  && (c[0] == 1 || c[0] == 3))
		{
			...
			System.arraycopy(c, 0, a, b.length + 1, c.length);
		} else if ((b[b.length - 1] > 0 && b[b.length - 1] < 13)
		  && ((c[0] == 14) || (c[0] == 16) || (c[0] == 20) || (c[0] == 22))
		  && (c[1] > 0 && c[1] < 13))
		{
			a = new byte[b.length + c.length + 1];
			System.arraycopy(b, 0, a, 0, b.length);
	...
	{
	//	////System.out.println("INSIDE BYTEMETH SUBARRAY");
	...
	public static boolean startsWith(byte[] b, byte[] c)
	{
		...
		{
			//////System.out.println(len1 + "            "+ UnicodeConverter.revert(c));
			if (b[i] == c[i])
			{
	...
	{
		//////System.out.println("INSIDE BYTEMETH  ENDSWITH:001");
	...
	{
		//////System.out.println("INSIDE BYTEMETH  ENDSWITH:002");
		int len1 = 0, len2 = 0;
